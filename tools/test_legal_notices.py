...
def main():
    ...
    assert "warn_t = warn_t + dt" in warn_update and \
        ...
        "the seconds must count down on warn_t"
    ...
    assert "brightness" not in warn_code, \
        "the consent button changes brightness over time again"
    ...
    assert "warn_wait - floor(warn_t)" in draw_warn and "warn_ready < 1" in draw_warn, \
        "the button has no countdown to unlocking"
    ...
    assert "line(" in outline and "outline_arc(" in outline, \
        "the outline must be strokes (line) on the sides plus arcs"
    ...
    assert "warn_frame_th" in config and "warn_frame_th" in draw_warn, \
        "the consent button has no outline (warn_frame_th)"
    arcs = re.findall(r"outline_arc\(x[^\n]*\)", outline)
    assert len(arcs) == 4, "the button needs four outline arcs on its corners"
    for mx, my in (("-1", "-1"), ("1", "-1"), ("1", "1"), ("-1", "1")):
        assert any(mx in arc for arc in arcs) and any(my in arc for arc in arcs), \
            "the arcs must cover all four quadrants"
    ...
    assert push and "legal" not in push.group(0).split("snprintf(body")[1], \
        "the consent stamp must not reach the cloud"
    net_h = read("net.h")
    ...
    for i in range(1, 11):
        assert "tr_pv%d()" % i in draw_privacy, "line tr_pv%d is not drawn" % i
    ...
    assert re.search(r"hit_settings_row\(x, y, 6\) == 1 then\s*\n\s*start_transition\(ST_PRIVACY\)",
                     touch_settings), "the privacy row does not open the screen"
    ...
    assert re.search(r"if game_state == ST_PRIVACY then\s*\n\s*start_transition\(ST_SETTINGS\)",
                     back), "Back on the privacy screen does not return to settings"
    ...
    for name in ["tr_legal_accept", "tr_privacy", "tr_privacy_title"] + \
        ...
        assert "language == 1" in body, "%s: no Russian branch" % name
        returns = re.findall(r'return "([^"]+)"', body)
        assert len(returns) == 2 and returns[0] and returns[1], \
            "%s: RU and EN strings are both needed" % name
    ...
    print("RESULT: the epilepsy consent gate, the settings.dat stamp, the privacy "
          "screen and the permission list are all in place")
    return 0
