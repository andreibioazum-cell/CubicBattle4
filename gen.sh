#!/bin/bash
# Супер-быстрый генератор game.c из game.ds
...
void init(AAssetManager* assets) {
    G = T_new();
    T_set(G, "COLOR_BG", &(Val){1,.num=0xFFCCCCCC}, 1);
    T_set(G, "COLOR_PLAYER", &(Val){1,.num=0xFFEE7722}, 1);
    T_set(G, "COLOR_JOY", &(Val){1,.num=0xFF000000}, 1);
    T_set(G, "COLOR_TEXT", &(Val){1,.num=0xFFFFFFFF}, 1);
    
    Table* p = T_new();
    T_set(p, "x", &(Val){1,.num=screen_w/2.0}, 1);
    T_set(p, "y", &(Val){1,.num=screen_h/2.0}, 1);
    T_set(p, "angle", &(Val){1,.num=0}, 1);
    T_set(p, "speed", &(Val){1,.num=5}, 1);
    T_set(p, "size", &(Val){1,.num=40}, 1);
    T_set(G, "player", p, 3);
...
void touch(float x, float y, int action) {
    ...
    if (action == 1) { joy.dx=0; joy.dy=0; joy.ox=0; joy.oy=0; return; }
    if (dist > joy.r + 30) return;
    if (dist < 15) { joy.dx=0; joy.dy=0; joy.ox=0; joy.oy=0; return; }
...
echo "game.c generated in 0.01s"
