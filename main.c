...
/* --- starting the renderer on a window ---
 * A renderer that failed to start used to be given up on: init_done stayed 0,
 * the loop skipped every frame and nothing tried again - a black screen with
 * no music (the sound and the script start only after the renderer) and no
 * crash. Now a failed start is retried after 0.5, 1 and 2 s (drivers that are
 * still busy with the previous app, a window that is not sized yet), and if
 * every try fails the reason is drawn on the window by the CPU. */
...
static void gfx_start_failed(struct android_app *app, const char *why) {
    ...
    if (gfx_start_tries < GFX_START_TRIES) {
        ...
        ds_log_err("renderer start %d/%d failed (%s), next try in %u ms", gfx_start_tries, GFX_START_TRIES,
                   why, (unsigned)(delay / 1000000ull));
        return;
    }
    gfx_retry_at_ns = 0;
...
static void start_window(struct android_app *app) {
    ...
    if (phys_w <= 0 || phys_h <= 0) { gfx_start_failed(app, "window size 0"); return; }
    apply_screen_size();
    ...
    if (!ds_graphics_init(script_assets, app->window)) { gfx_start_failed(app, ds_graphics_failure()); return; }
    if (gfx_start_tries) ds_log("renderer started on try %d", gfx_start_tries + 1);
    gfx_start_tries = 0; gfx_retry_at_ns = 0; gfx_failure_shown = 0;
    ...
    (void)start_script(script_started_once);
}
static void handle_cmd(struct android_app *app, int32_t command) {
    ...
    switch (command) {
        case APP_CMD_INIT_WINDOW:
            /* A new window: a failed start on the previous one does not count. */
            gfx_start_tries = 0; gfx_retry_at_ns = 0; gfx_failure_shown = 0;
            start_window(app); break;
        ...
        case APP_CMD_TERM_WINDOW:
            ...
            init_done = 0; keyboard_hide(); gfx_retry_at_ns = 0; gfx_failure_shown = 0;
            ds_graphics_window_lost(); ds_sound_suspend(); break;
        case APP_CMD_WINDOW_REDRAW_NEEDED:
            /* The failure screen lives in the window buffer; draw it again if
             * the system asks (rotation, the window uncovered). */
            if (gfx_failure_shown && app->window) (void)ds_graphics_show_failure(app->window, gfx_start_tries);
            break;
 ...
 * would follow one and the same sequence every run. */
    ...
    for (;;) {
        ...
        int timeout = drawing ? (script_active ? 0 : 10) : (gfx_retry_at_ns ? 50 : 250);
