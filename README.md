...
### 5. Онлайн: меньше лага и корректные пули
- **Меньше лага:** `net.c` больше не делает последовательные HTTP-запросы на одном
  ...
  параллельных потоках с интервалом 60 мс вместо 100 мс — обновления соперников
  приходят в ~3–4 раза чаще и не блокируют друг друга.
- **Пули из ствола, а не «из ниоткуда»:** раньше пуля соперника создавалась в его
  *текущей* позиции с `traveled=0`, то есть появлялась уже в полёте и могла даже
  возникнуть уже за целью — из-за чего урон иногда не начислялся. Теперь клиент
  публикует точку выстрела (`x`,`y`), направление и пройденный путь (`tr`), а все
  восстанавливают полную траекторию `позиция = x + dx*tr`. Пули летят из ствола и
  одинаково задевают всех на своём пути.
- `rbx()`/`rby()` — единая функция позиции чужой пули (вместо трёх повторений).
...
├── battle.ds    132 — соло + синхронизированный онлайн Firebase (пули по траектории)
