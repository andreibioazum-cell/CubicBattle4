...
- **Удар вместо пули:** по кнопке атаки пуля больше не вылетает — показывается
  прицел, спрайт игрока на ~0.2 с меняется с `ordinary.png` на `ordinary_punch.png`,
  и срабатывает хитбокс ближнего боя: широкий (`punch_width` = 90px) и недлинный
  (`punch_reach` = 70px). Событие удара публикуется в Firebase как
  `(x, y, dx, active, shot)` — соперники показывают спрайт удара и получают урон,
  если стоят в хитбоксе.
