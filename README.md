...
Окно Android лочится один раз на кадр, скрипт собирает команды, `graphics.c` растеризует их в правильном порядке. `rect`/`circle`/`ring` — быстрые span-заливки; непрозрачные PNG в масштабе 1:1 копируются через `memcpy`. Никаких OpenGL/EGL.
