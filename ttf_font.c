...
DSFont *ds_font_create(const uint8_t *data, size_t size, int ph) {
    ...
    /* Полный набор символов, которые использует игра: все строки интерфейса
     * (включая японские) плюс вся хирагана/катакана, чтобы текст, который
     * вводит игрок (чат), тоже отрисовывался. Раньше здесь был только
     * фиксированный список — японские строки экрана аккаунта/логина и любой
     * введённый японский текст не имели глифов и не отображались. */
    if (!add_utf8(f,
        "–…←↑→↓、。「」『』〜ぁあぃいぅうぇえぉおかがきぎくぐけげこごさざしじすずせ"
        "ぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃ"
        "やゅゆょよらりるれろゎわゐゑをんゔゕゖ゛゜ァアィイゥウェエォオカガキギクグケゲコ"
        "ゴサザシジスズセゼソゾタダチヂッツヅテデトドナニヌネノハバパヒビピフブプヘベペホ"
        "ボポマミムメモャヤュユョヨラリルレロヮワヰヱヲンヴヵヶヷヸヹヺ・ーヽヾ上下中信倒違以作成既存在再一持致"
        "入利力勝北去定戻手接撃攻敗日本消相続設語送遊閉")) {
