...
```json
{
  "rules": {
    ...
    "accounts": {
      "$nicknameHash": {
        ".read": true,
        ".write": "!data.exists() && newData.hasChildren(['nickname', 'salt', 'verifier', 'created'])",
        "nickname": {
          ".validate": "newData.isString() && newData.val().length >= 3 && newData.val().length <= 48"
        },
        "salt": {
...
> Старые правила только с `rooms/main` больше не подходят: без раздела
> `accounts` игра покажет ошибку Firebase при сохранении ника.

До 4 игроков в одной папке `rooms/main`. Клиент сам занимает первый свободный
слот (`players/0` ... `players/3`), поэтому экран ожидания не нужен. Каждый ник
регистрируется один раз: запись аккаунта после создания нельзя перезаписать
другим паролем.
...
Не оставляй так навсегда: любой, кто знает URL, сможет стереть базу, заменить
запись аккаунта и тем самым отключить гарантию уникального ника.
...
```
accounts/
  ХЕШ_НИКА/  nickname, salt, verifier, created

rooms/
  main/
    players/
      0/  uid, nickname, x, y, angle, hp, alive, seq
      1/  ...
      ...
...
В `accounts` пароль не хранится открытым текстом: клиент сохраняет случайную
соль и результат PBKDF2-HMAC-SHA256 (60 000 итераций). На самом устройстве
остаётся только закрытый файл профиля с солью и проверочным значением, без
исходного пароля. Ник нормализуется без учёта регистра, поэтому `Player` и
`player` считаются одним аккаунтом. Если пароль не совпал, онлайн-режим не
запускается — нужно ввести правильный пароль или выбрать свободный ник.
