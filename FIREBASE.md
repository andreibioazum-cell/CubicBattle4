...
«Онлайн» без входа игра сама откроет экран «Ник и пароль»:

- **Ник 3–16 символов** (латиница, цифры, `_`), **пароль от 4 символов**.
- Если аккаунта с таким ником ещё нет — он **создаётся автоматически**
  (это и есть регистрация): в `accounts/<ник>/` появляются `salt`, `hash`,
  `created`.
- Если аккаунт уже есть — игра проверяет пароль. **Одинаковый ник с другим
  паролем не пустит**: будет «Неверный пароль».
- Создание аккаунта атомарное (`if-match: null`): если два человека одновременно
  регистрируют один ник, второй получит 412 и просто попробует войти.
- После входа сессия сохраняется в файл `auth.dat` (внутренняя память приложения)
  и при следующем запуске игрок входит автоматически. Кнопка «Выйти» есть на
  экране «Аккаунт».

Код входа (`net_login`, `net_autologin`, `net_logout`, SHA-256-хеши) живёт в
`net.c`, экран — в `game/menu.ds`.
