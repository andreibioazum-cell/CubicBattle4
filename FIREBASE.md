...
`x`, `y` — точка выстрела (ствол), `dx`, `dy` — направление, `tr` — пройденный
путь. Текущая позиция пули = `x + dx*tr`. Клиенты восстанавливают траекторию из
этих данных, поэтому пули летят из ствола соперника, а не появляются «из ниоткуда»
и одинаково наносят урон всем, кого задели.
